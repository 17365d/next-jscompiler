...
define(function (require, exports, module) {
    ...
    function compileJS() {
        ...
        var currentFile = DocumentManager.getCurrentDocument().file,
            ...
            regex = new RegExp('/[^/]*$'),
            parentDir = directory.substring(0, directory.length - 1).replace(regex, '/'),
            options,
            preferences = null;
            ...
            if (err) {
                if (err === 'NotFound') {
                    ...
                    if (preferences !== undefined) {
                        ...
                        FileSystem.getFileForPath(parentDir + '.jscompiler.json').read({}, function (err, content) {
                            var ext = currentFile.name.split('.').pop();
                            if (!err) {
                                if (ext === 'js') {
                                    options = JSON.parse(content);
                                    compileWithOptions(options, parentDir);
                                }
                            } else {
    ...
    CommandManager.register('Force Compress', 'jscompiler.force', function () {
        forceCompile = true;
        compileJS();
    });
    KeyBindingManager.addBinding('jscompiler.force', command);
    ...
    $(DocumentManager).on("documentSaved", function (event, doc) {
        /*
        ** #status-inspection
        * .inspection-valid / .inspection-errors
        */
        if (prefs.get("on-save")) {
            var directory = DocumentManager.getCurrentDocument().file.parentPath,
                ext = DocumentManager.getCurrentDocument().file.name.split('.').pop(),
                regex = new RegExp('/[^/]*$'),
                parentDir = directory.substring(0, directory.length - 1).replace(regex, '/'),
                options;

            FileSystem.getFileForPath(directory + '.jscompiler.json').read({}, function (err) {
                if (!err) {
                    if (ext === 'js') {
                        compileJS();
                    }
